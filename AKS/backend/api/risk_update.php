...
if ($risk_id <= 0 || $company_id <= 0 || empty($title)) {
    send_error("Risk ID, Company, and Risk Title are required.");
}
...
$stmt = $conn->prepare("
    UPDATE risks 
    SET company_id = ?, risk_title = ?, risk_description = ?, likelihood = ?, impact = ?, risk_score = ?, mitigation_strategy = ?, status = ? 
    WHERE risk_id = ?
");
$stmt->bind_param("issiiissi", $company_id, $title, $description, $likelihood, $impact, $risk_score, $mitigation, $status, $risk_id);
...
if ($stmt->execute()) {
    ...
    send_error("Database failure: failed to update risk entry.");
}
?>
